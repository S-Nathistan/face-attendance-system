...
def initialize_models():
    ...
    try:
        ...
        print("MobileFaceNet model for temp image is loaded successfully.")

    except Exception as e:
        initialization_error = str(e)
        print(f"temp image model Initialization failed: {initialization_error}")
...
def get_face_embedding(face_img):
    try:
        ...
        print(f"temp image Embedding error: {e}")
        return None

class AddEmployeeTempEmbedding(Resource):
    def post(self):
        if not initialization_complete:
            if initialization_error:
                return jsonify({"status": "error", "error": initialization_error}), 500
            return jsonify({"status": "error", "error": "temp image Models still initializing"}), 503
        ...
        try:
            ...
            # Step 1: Get the current embeddings for the employee to check emp_no 6,7,8
            cursor.execute("SELECT emb_no FROM tblEmbeddings WHERE emp_id = ? AND emb_type = 'temp'", (emp_id,))
            existing_temp_embeddings = cursor.fetchall()

            # Step 2: Find the next available emp_no (6, 7, or 8)
            temp_numbers = {6, 7, 8}
            existing_temp_numbers = {emb[0] for emb in existing_temp_embeddings}
            available_temp_numbers = temp_numbers - existing_temp_numbers
            next_temp_number = min(available_temp_numbers) if available_temp_numbers else None

            if next_temp_number is None:
                return jsonify({"status": "fail", "error": "All temporary slots (6, 7, 8) are full"})
            ...
            for idx, image_b64 in enumerate(all_images):
                try:
                    ...
                    debug_save_path = os.path.join(os.path.dirname(__file__), f'received_temp_image_face_for_embedding{idx}.jpg')
                    ...
                    debug_save_path_after_resize = os.path.join(os.path.dirname(__file__), f'received_temp_image_face_for_embedding_after_resize_from_backend{idx}.jpg')
                    ...
                    # Save embedding with `emp_type` as 'temp' and the next available `emp_no`
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, GETDATE(), ?, ?)
                        """,
                        (emp_id, embedding_bytes, next_temp_number, 'temp')
                    )
                    embeddings_inserted += 1

                    # Update next available number for the next image (if multiple images)
                    next_temp_number = next_temp_number + 1 if next_temp_number < 8 else None
                    if next_temp_number is None:
                        break
